...
public:
    ...
    friend std::ostream& operator<<(std::ostream& lhs, const rational& rhs){
        return lhs << std::showbase << std::hex << "[" << rhs.counter << "/" << rhs.denominator << "]";
    }
    ...
    {
        int temp_c = counter * rhs;
//        int temp_d = denominator * rhs;
        return rational(temp_c, denominator);
    }
    ...
    /// \brief
    /// multiply a rational into another rational
    /// \details
    /// This operator*= multiplies a rational value into rational variable.
    rational operator*=(const rational& rhs)
    {
