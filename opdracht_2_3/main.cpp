...
TEST_CASE( "add vector by vector" ){
   vector v( 3, 4 );
   REQUIRE( v + vector(1,2) == vector(4,6) );   
}
...
TEST_CASE( "add vector into another vector" ){
   vector v( 3, 4 );
   v+=vector(1,1);
...
TEST_CASE( "multiply vector by integer" ){
   vector v( 3, 4 );
   REQUIRE( v*2 == vector(6,8) );   
}

TEST_CASE( "multiply vector by vector" ){
   vector v( 3, 4 );
   
   REQUIRE( v * vector(2, 1)==  vector(6,4));   
}
TEST_CASE( "multiply vector into another vector" ){
   vector v( 3, 4 );
   v*=vector(3,2);
   REQUIRE( v ==  vector(9,8));   
}
